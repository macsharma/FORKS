...
#%%
dataframe=pd.read_csv("data/data.csv",sep=",",header=None)
#data in form samples X features
data=dataframe.iloc[0:,0:].as_matrix().T
...
actual_time=pd.read_csv("data/actual_time.csv",sep=",",header=None)
actual_time=np.array(actual_time.iloc[0:,0],dtype='int32')
#%%
gene_name=pd.read_csv("data/gene_names.csv",sep=",",header=None)
cell_name=pd.read_csv("data/cell_names.csv",sep=",",header=None)
gene_name=gene_name.iloc[0:,0]
cell_names=cell_name.iloc[0:,0]
#gene_name=pd.Series(gene_name.unique())
#%%
actual_time=actual_time[cell_list]
gene_name=gene_name[gene_list]
cell_names=cell_names[cell_list]
...
#%%
#tmp_dir_path =path1+'\\temp'
#dev_null=open(devnull, 'w') 
#tt=init_bh_tsne(data, tmp_dir_path, no_dims=2, perplexity=2, theta=0.5, randseed=1,verbose=True, initial_dims=150, use_pca=True, max_iter=1000)
#tt12=isinstance(data, file)
#res = []
#for result in bh_tsne(tmp_dir_path, verbose=True):
#    sample_res = []
#    for r in result:
#        sample_res.append(r)
...
#%%
actual_time1=np.array(actual_time1)
...
inds = np.where(cum_sum_exp_var > 0.95)
red_dim=inds[0][0]
...
#all_mapping_type=['SpectralEmbedding']
init_cell=list([])
